...
split_ratio = 0.67
training_data, test_data = split_data(split_ratio)
num_of_chromosomes = 5
chromosomes = [[random.choice([0,1]) for i in range(num_features)] for j in range(num_of_chromosomes)]
new_datasets = []
...
for i in range(num_of_chromosomes):
    ...
    for data in training_data:
        ...
        temp.append(data[-1])
        temp1.append(temp)
    new_datasets.append(temp1)

print(new_datasets)

#Evaluate chromosomes
for chromosome_dataset in new_datasets:
    prob_features,prob_features_given_yes_attribute_true,prob_features_given_yes_attribute_false,prob_features_given_no_attribute_true,prob_features_given_no_attribute_false = probability_of_features(chromosome_dataset)
    prob_result = prob_yes_or_no(chromosome_dataset)
    chromosome_accuracy = test(test_data)
